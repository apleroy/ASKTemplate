...
namespace ASK.Services
{
    ...
    public static class AlexaRequestSignatureVerifierService
    {
        ...
        public static bool VerifyCertificateUrl(string certChainUrl)
        {
            ...
            return
                certChainUri.Host.Equals(AlexaSdk.SIGNATURE_CERT_URL_HOST, StringComparison.OrdinalIgnoreCase) &&
                certChainUri.PathAndQuery.StartsWith(AlexaSdk.SIGNATURE_CERT_URL_PATH) &&
        ...
        public static bool CheckRequestSignature(byte[] serializedSpeechletRequest, string expectedSignature, X509Certificate cert)
        {
            ...
            var signer = Org.BouncyCastle.Security.SignerUtilities.GetSigner(AlexaSdk.SIGNATURE_ALGORITHM);
        ...
        private static bool CheckCertSubjectNames(X509Certificate cert)
        {
            ...
            for (int i = 0; i < subjectNamesList.Count; i++)
            {
                ...
                {
                    if (subjectNames[j] is String && subjectNames[j].Equals(AlexaSdk.ECHO_API_DOMAIN_NAME))
                    {
                        found = true;
                        break;
                    }
